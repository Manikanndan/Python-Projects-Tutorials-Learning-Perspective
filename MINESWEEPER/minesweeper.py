...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        # initialize a set to keep track of which locations we're uncovered
    ...
    def make_new_board(self):
        ...
        while bombs_planted < self.num_bombs:
            ...
            if board[row][col] == "*":
                # this means we're actually planted a bomb there already so keep going
                continue
    ...
    def assign_values_to_board(self):
        # now that we have tyhe bombs planted, let's assign a number 0-8 for all the empty spaces, which 
    ...
    def dig(self, row, col):
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1):
                ...
                self.dig(r, c)
        # if our initial dig didn't hit the bomb, we *shouldn't* hit a bomb here 
        return True 
         
    def __str__(self):
        # this is a magic function where if you call print on thid object,
...
def play(dim_size=10, num_bombs=10):
    ...
    # step 3b: if location is not a bomb, dig recursively untill each sqaure is at least next to a bomb
