...
def solve_sudoku(puzzle):
    ...
    # our puzzle is a lists, where each inner list is a row in our sudoku puzzle
    ...
    # step 1.1: if there's nowwhere left, then we're done because we only allowed valid inputs
    if row is None: # this is true if our find_next_empty function returns None, None
        return None

    # step 2: if there is a place to put a number, then make a guess between 1 and 9
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, .... 9
        # step 3: check if there is valid guess
        if is_valid(puzzle, guess, row, col):
            # step 3.1: if this is a valid guess, then place it at that spoton the puzzle
        ...
        # step 5: it not valid or if nothing gets returned true, then we need to backtrack and try a new numner
        puzzle[row][col] = -1
